        ...
        <div class="row justify-content-center w-100 pt-4 mr-0 ml-0">
          <ul class="list-group w-100">
            ...
            if(isset($_SESSION['username'])){
              if ($stmt = $conn->prepare("SELECT name, surname, birthdate, phoneNumber, city, employment FROM user WHERE email = ? LIMIT 1")) {
                $stmt->bind_param('s', $_SESSION['username']);
                // Eseguo la query creata.
                $stmt->execute();
                $stmt->bind_result($name, $surname, $birthdate, $phoneNumber, $city, $employment);
                $stmt->store_result();
                $stmt->fetch();
                if($stmt->num_rows == 1) {
                  echo '
